...
RGradientLabel::RGradientLabel(QWidget *parent)
    : QWidget(parent)
{
    m_step = 10;
    m_interval = 100;
...
void RGradientLabel::setStep(int step)
{
    if (m_step != step) {
        m_step = step;
        update();
...
void RGradientLabel::setInterval(int interval)
{
...
void RGradientLabel::setBgColor(const QColor &bgColor)
{
...
QSize RGradientLabel::sizeHint() const
{
    return QSize(180, 180);
}


void RGradientLabel::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
...
void RGradientLabel::drawBg(QPainter *painter)
{
    int radius = 99;
    painter->save();
