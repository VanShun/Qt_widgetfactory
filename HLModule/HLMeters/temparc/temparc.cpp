...
TempArc::TempArc(QWidget *parent)
    : QWidget(parent)
{
    m_startAngle = 240;  //0刻度的起始角度
    m_endAngle = 300;
    m_foreground = QColor(0x64, 0x95, 0xed);
    m_background = QColor(0xdcdcdc); //定义背景颜色
    m_minValue = 18;
    m_maxValue = 30;
    m_scaleMajor = 12;//刻度
...
void TempArc::drawArc(QPainter *painter)
{
    ...
    pen.setColor(QColor(0xdc, 0xdc, 0xdc));
    painter->setPen(pen);
    if (m_showsvalue) {
...
void TempArc::setForeground(const QColor &color)
{
    m_foreground = color;
    update();
}

void TempArc::setBackground(const QColor &color)
{
    m_background = color;
    update();
}
